...
def get_sections_new(path,inference=False):
    ...
    label_file_name = path.name.split(".")[0]+".json"
    label_path=os.path.join(label_path_prefix, label_file_name)

    # file = open(str(path), "r")
    # raw_content = json.load(file)
    # file.close()
    #
    # sentence_embedding = numpy.asarray(raw_content["array"])
    sentence_embedding = load(str(path))
...
def read_wiki_file(path, word2vec, inference=False, remove_preface_segment=False, ignore_list=False, remove_special_tokens=False,
    ...
    data = []
    targets = []
    #path = "/home/anustup/Desktop/Mediadistillery/Chapter_detection/data/dutch_data/train_main/data/b54d5644-8c30-3a17-87a8-d4a7d75036a3.json"
...
class CustomDutchEmbDataset(Dataset):
