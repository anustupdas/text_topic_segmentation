...
def extract_sentence_words(sentence, remove_missing_emb_words = False,remove_special_tokens = False):
    ...
    setnece_without_punctuation = sentence.translate(str.maketrans('', '', string.punctuation))
    sentence_words += [x.lower() for x in setnece_without_punctuation.split(" ") if len(x) > 2]
...
def word_model(word, model):
    if model is None:
        ...
        if word in model:
            ...
            return model['vogel'].reshape(1, 300)
