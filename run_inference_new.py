...
def main(args):
    ...
    # test_dataset = CustomDutchEmbDataset(args.infer, word2vec=word2vec, inference=True,
    #                                   high_granularity=args.high_granularity)
    ...
    with open(os.path.join(args.infer,'result_embedding.json'), 'w') as fp:
        json.dump(result, fp)
    print(f"Results saved in {args.infer} as result_embedding.json")
