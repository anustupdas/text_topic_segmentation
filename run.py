        ...
        for i, (data, target, paths) in enumerate(dataset):
            if True:
                if i == args.stop_after:
                    break
                # print(f"Batched dat path {paths[0]}")
                # text_file = open("/home/anustup/Desktop/Mediadistillery/Chapter_detection/data/dutch_data/Goodfiles_1.txt", "a")
                # text_file.write("\n"+paths[0].name)
                # text_file.close()
...
def main(args):
    ...
    if not args.infer:
        if args.wiki:
            ...
            train_dataset = CustomDutchDataset(dataset_path / 'train_small/data', word2vec=word2vec,
                                                  high_granularity=args.high_granularity)
            dev_dataset = CustomDutchDataset(dataset_path / 'dev/data', word2vec=word2vec, high_granularity=args.high_granularity)
            test_dataset = CustomDutchDataset(dataset_path / 'test/data', word2vec=word2vec,
                                                 high_granularity=args.high_granularity)

            # train_dataset = CustomDutchEmbDataset(dataset_path / 'train/emb_small', word2vec=word2vec,
            #                                  high_granularity=args.high_granularity)
            # dev_dataset = CustomDutchEmbDataset(dataset_path / 'dev/emb_small', word2vec=word2vec, high_granularity=args.high_granularity)
            # test_dataset = CustomDutchEmbDataset(dataset_path / 'test/emb_small', word2vec=word2vec,
            #                                 high_granularity=args.high_granularity)
        ...
        # train_dl = DataLoader(train_dataset, batch_size=args.bs, collate_fn=collate_fn_emb, shuffle=True,
        #                       num_workers=args.num_workers)
        # dev_dl = DataLoader(dev_dataset, batch_size=args.test_bs, collate_fn=collate_fn_emb, shuffle=False,
        #                     num_workers=args.num_workers)
