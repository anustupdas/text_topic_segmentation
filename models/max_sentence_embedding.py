...
class Model(nn.Module):
    def __init__(self, sentence_encoder, hidden=128, num_layers=4):
        ...
        self.sentence_lstm = nn.LSTM(input_size=sentence_encoder.hidden * 2,
                                     hidden_size=hidden,
                                     num_layers=num_layers,  # earlier it was 2
                                     batch_first=True,
                                     dropout=0,
                                     bidirectional=True)
        ...
        self.criterion = nn.CrossEntropyLoss(weight=torch.Tensor([1,3]))
